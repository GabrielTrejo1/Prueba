...
class Ventas():
  ...
  def initGui(self): # lo que está en esta función se va a ejecutar al iniciar el programa
    try:
  ...
  def cargar_clientes(self):
    ...
    for item in datos_clientes:
      self.ventas.tblClientes.setItem(fila,0,QTableWidgetItem(str(item[0])))
      self.ventas.tblClientes.setItem(fila,1,QTableWidgetItem(str(item[1])))
      self.ventas.tblClientes.setItem(fila,2,QTableWidgetItem(str(item[2])))
      fila +=1
    cursor.close()
